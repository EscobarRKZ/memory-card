(() => {
  ...
  const getApi=()=>window.MemoryCardBackendV2?.configured?window.MemoryCardBackendV2:null;
  ...
  function friendlyError(error){
    const raw=String(error?.message||error||'Ошибка Memory Card Network');
    if(raw.includes('USER_NOT_FOUND'))return new Error('Пользователь с таким Memory Card ID не найден');
    if(raw.includes('SELF_REQUEST'))return new Error('Нельзя добавить самого себя');
    if(raw.includes('NOT_FRIENDS'))return new Error('Профиль доступен только друзьям');
    if(raw.includes('NOT_AUTHENTICATED'))return new Error('Нужно войти в аккаунт');
  ...
  async function waitApi(){
    ...
    throw new Error('Облачный профиль ещё не готов. Попробуй ещё раз через пару секунд.');
  }
  ...
  async function ensureIdentity(){
    ...
    if(profile){
      state.settings.userId=String(profile.memory_id||state.settings.userId||'').toUpperCase();
      state.settings.profileName=String(profile.display_name||state.settings.profileName||'Игрок');
      state.social.profile={userId:state.settings.userId,displayName:state.settings.profileName};
    }
    return Boolean(state.settings.userId);
  }

  function renderFriends(){
    if(!getApi())return `<div class="section-head"><div><h2>Друзья</h2><p>Подключаю Memory Card Network…</p></div></div><section class="panel"><div class="social-loader">Загружаю облачный профиль…</div></section>`;
    ...
    if(!state.settings.userId)return `<div class="section-head"><div><h2>Друзья</h2><p>Получаю твой профиль…</p></div></div><section class="panel"><div class="social-loader">Получаю уникальный Memory Card ID…</div></section>`;
  ...
  function renderFriendProfile(){
    const fp=state.friendProfile;
    if(!fp)return renderFriends();
  ...
  async function refreshSocial(quiet=false){
    ...
    try{
      ...
      if(!quiet)setTaskProgress('Друзья',15,100,'Обновляю список');
      const data=await rpc('social_state');
      Object.assign(state.social,{profile:data?.profile||state.social.profile,friends:data?.friends||[],incoming:data?.incoming||[],outgoing:data?.outgoing||[],loaded:true,error:''});
      if(!quiet)finishTaskProgress('Друзья обновлены',`${state.social.friends.length} друзей`);
      render();
    }catch(error){
      const e=friendlyError(error);
      state.social.error=e.message;
      if(!quiet)failTaskProgress('Не удалось обновить друзей',e.message);
      render();
    }finally{
  ...
  async function addFriend(event){
    ...
    const id=String(new FormData(event.currentTarget).get('friendId')||'').trim().toUpperCase();
    if(!id)return;
    try{
      setTaskProgress('Приглашение',20,100,`Ищу ${id}`);
      await rpc('send_friend_request',{target_memory_id:id});
      finishTaskProgress('Приглашение отправлено',id);
      await refreshSocial(true);
      setToast('Приглашение отправлено');
    }catch(error){
      const e=friendlyError(error);
      failTaskProgress('Не удалось отправить',e.message);
      alert(e.message);
    }
  }
  ...
  async function respond(id,accept){
    try{
      const ok=accept?await rpc('accept_friend_request',{friendship_id:id}):await rpc('reject_friend_request',{friendship_id:id});
      if(ok!==true)throw new Error('Заявка уже обработана или недоступна');
      await refreshSocial(true);
      setToast(accept?'Друг добавлен':'Приглашение отклонено');
    }catch(error){alert(friendlyError(error).message);}
  }

  async function openFriend(id){
    state.friendProfile={loading:true,userId:id};
    render();
    try{
      const data=await rpc('get_friend_profile',{target_memory_id:id});
      if(!data)throw new Error('Профиль не найден');
      state.friendProfile={loading:false,profile:data.profile,games:data.games||[],stats:data.stats||{}};
    }catch(error){state.friendProfile={loading:false,error:friendlyError(error).message,userId:id};}
    render();
  }

  async function removeFriend(id){
    if(!confirm('Удалить этого пользователя из друзей? После этого профили снова станут недоступны друг другу.'))return;
    try{
      const ok=await rpc('remove_friend',{target_memory_id:id});
      if(ok!==true)throw new Error('Не удалось удалить пользователя из друзей');
      state.friendProfile=null;
      await refreshSocial(true);
      setToast('Удалено из друзей');
    }catch(error){alert(friendlyError(error).message);}
  }

  async function saveName(name){
    name=String(name||'').trim().slice(0,40)||'Игрок';
  ...
  navButtons=function(){
    ...
    return items.map(([view,label])=>`<button data-nav="${view}" class="${state.view===view?'active':''}">${label}${view==='friends'&&state.social.incoming.length?`<span class="nav-badge">${state.social.incoming.length}</span>`:''}</button>`).join('');
  };
  ...
  navigate=function(view){
    state.friendProfile=null;
    baseNavigate(view);
    if(view==='friends')setTimeout(()=>refreshSocial(true),20);
  };
  ...
  bind=function(){
    ...
    document.querySelectorAll('[data-action="friend-accept"]').forEach(x=>x.onclick=()=>respond(x.dataset.requestId,true));
    document.querySelectorAll('[data-action="friend-reject"]').forEach(x=>x.onclick=()=>respond(x.dataset.requestId,false));
    document.querySelectorAll('[data-friend-open]').forEach(x=>x.onclick=()=>openFriend(x.dataset.friendOpen));
    document.querySelectorAll('[data-action="friend-back"]').forEach(x=>x.onclick=()=>{state.friendProfile=null;render();});
    document.querySelectorAll('[data-action="friend-remove"]').forEach(x=>x.onclick=()=>removeFriend(x.dataset.friendId));
    const form=document.querySelector('#friendAddForm');
    if(form)form.onsubmit=addFriend;
    const name=document.querySelector('#profileName');
    if(name)name.onchange=()=>saveName(name.value);
  };

  window.MemoryCardSocialV2={refresh:refreshSocial,openFriend};
})();