const MC={V:13,G:'Games',P:'Profiles',D:'ProfileDevices',US:'UserSettings',F:'Friends',S:'Stats',LS:'Settings',OWNER:'MEMORY_CARD_LEGACY_OWNER_ID'};
const GH=['id','platform','title','releaseYear','genre','franchise','coverUrl','status','rating','replay','notes','addedAt','updatedAt','deletedAt'];
const MH=['userId'].concat(GH),PH=['userId','displayName','createdAt','updatedAt'],DH=['tokenHash','userId','createdAt','lastSeenAt'],UH=['userId','settingsJson','updatedAt'],FH=['requestId','fromUserId','toUserId','status','removedByUserId','createdAt','updatedAt'];
const SK=['theme','language','reducedMotion','handheldRotation','desktopRotation','currentHandheld','currentDesktop','ownedPlatforms','rotationHandheldEnabled','rotationDesktopEnabled','settingsUpdatedAt'];
...
function bridgeRequest(p){try{p=p||{};const a=String(p.action||'');if(a==='ping')return ping_(p);if(a==='sync')return legacySync_(p);if(a==='profileBootstrap')return bootstrap_(p);if(a==='profileSync')return profileSync_(p);if(a==='profileUpdate')return profileUpdate_(p);if(a==='socialState')return socialAction_(p);if(a==='friendRequest')return friendRequest_(p);if(a==='friendRespond')return friendRespond_(p);if(a==='friendRemove')return friendRemove_(p);if(a==='friendDismiss')return friendDismiss_(p);if(a==='friendProfile')return friendProfile_(p);return{ok:false,error:'Unknown action'};}catch(x){return{ok:false,error:err_(x)};}}
...
function socialAction_(p){const ss=SpreadsheetApp.getActiveSpreadsheet(),id=require_(ss,p),s=social_(ss,id);return{ok:true,profile:publicProfile_(profile_(ss,id),userSettings_(ss,id)),friends:s.friends,incoming:s.incoming,outgoing:s.outgoing,removed:s.removed};}

function friendRequest_(p){return locked_(()=>{const ss=SpreadsheetApp.getActiveSpreadsheet(),id=require_(ss,p),fid=uid_(p.friendId);if(!fid)return{ok:false,error:'Укажи Memory Card ID'};if(fid===id)return{ok:false,error:'Нельзя добавить самого себя'};const target=profile_(ss,fid);if(!target)return{ok:false,error:'Пользователь с таким ID не найден'};let r=friends_(ss);if(r.some(x=>x.status==='accepted'&&pair_(x,id,fid)))return{ok:false,error:'Этот пользователь уже у тебя в друзьях'};if(r.some(x=>x.status==='pending'&&x.fromUserId===fid&&x.toUserId===id))return{ok:false,error:'Этот пользователь уже отправил тебе приглашение. Прими его во входящих.'};r=r.filter(x=>!(x.status==='removed'&&pair_(x,id,fid)));let x=r.find(x=>x.fromUserId===id&&x.toUserId===fid),n=new Date().toISOString();if(x&&x.status==='pending')return{ok:true,target:baseProfile_(target),requestId:x.requestId,alreadyPending:true};if(x){x.status='pending';x.removedByUserId='';x.updatedAt=n;}else{x={requestId:Utilities.getUuid(),fromUserId:id,toUserId:fid,status:'pending',removedByUserId:'',createdAt:n,updatedAt:n};r.push(x);}writeFriends_(ss,r);return{ok:true,target:baseProfile_(target),requestId:x.requestId};});}
function friendRespond_(p){return locked_(()=>{const ss=SpreadsheetApp.getActiveSpreadsheet(),id=require_(ss,p),r=friends_(ss),x=r.find(x=>x.requestId===String(p.requestId||'')&&x.toUserId===id&&x.status==='pending');if(!x)return{ok:false,error:'Приглашение не найдено или уже обработано'};x.status=p.accept?'accepted':'rejected';x.removedByUserId='';x.updatedAt=new Date().toISOString();writeFriends_(ss,r);return{ok:true,accepted:x.status==='accepted'};});}
function friendRemove_(p){return locked_(()=>{const ss=SpreadsheetApp.getActiveSpreadsheet(),id=require_(ss,p),fid=uid_(p.friendId),r=friends_(ss);let ok=false;r.forEach(x=>{if(x.status==='accepted'&&pair_(x,id,fid)){x.status='removed';x.removedByUserId=id;x.updatedAt=new Date().toISOString();ok=true;}});if(!ok)return{ok:false,error:'Дружба не найдена'};writeFriends_(ss,r);return{ok:true};});}
function friendDismiss_(p){return locked_(()=>{const ss=SpreadsheetApp.getActiveSpreadsheet(),id=require_(ss,p),fid=uid_(p.friendId),r=friends_(ss);const next=r.filter(x=>!(x.status==='removed'&&pair_(x,id,fid)&&x.removedByUserId&&x.removedByUserId!==id));if(next.length===r.length)return{ok:false,error:'Уведомление уже удалено'};writeFriends_(ss,next);return{ok:true};});}
...
function social_(ss,id){const pm={};profiles_(ss).forEach(p=>pm[p.userId]=p);const rows=friends_(ss),games=allGames_(ss),friends=[],incoming=[],outgoing=[],removed=[];rows.forEach(x=>{if(x.status==='accepted'&&(x.fromUserId===id||x.toUserId===id)){const oid=x.fromUserId===id?x.toUserId:x.fromUserId,p=pm[oid];if(!p)return;const st=gameStats_(games.filter(g=>g.userId===oid&&!g.deletedAt));friends.push({userId:oid,displayName:p.displayName,completed:st.completed,playing:st.playing,averageRating:st.averageRating});}else if(x.status==='pending'&&x.toUserId===id){const p=pm[x.fromUserId];if(p)incoming.push({requestId:x.requestId,userId:p.userId,displayName:p.displayName,createdAt:x.createdAt});}else if(x.status==='pending'&&x.fromUserId===id){const p=pm[x.toUserId];if(p)outgoing.push({requestId:x.requestId,userId:p.userId,displayName:p.displayName,createdAt:x.createdAt});}else if(x.status==='removed'&&(x.fromUserId===id||x.toUserId===id)&&x.removedByUserId&&x.removedByUserId!==id){const p=pm[x.removedByUserId];if(p)removed.push({requestId:x.requestId,userId:p.userId,displayName:p.displayName,updatedAt:x.updatedAt});}});friends.sort((a,b)=>a.displayName.localeCompare(b.displayName));incoming.sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));outgoing.sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));removed.sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt)));return{friends,incoming,outgoing,removed};}
...
function friends_(ss){return readObjs_(sheet_(ss,MC.F,FH)).map(x=>({requestId:String(x.requestId||''),fromUserId:uid_(x.fromUserId),toUserId:uid_(x.toUserId),status:String(x.status||''),removedByUserId:uid_(x.removedByUserId),createdAt:String(x.createdAt||''),updatedAt:String(x.updatedAt||'')})).filter(x=>x.requestId&&x.fromUserId&&x.toUserId);}
