/* Memory Card catalog refresh helper — v0.21
   Refreshes only consoles selected in "Мои приставки".
...
(() => {
  ...
  function selectedPlatforms() {
    try {
      ...
      if (typeof state !== 'undefined' && Array.isArray(state?.settings?.ownedPlatforms) && state.settings.ownedPlatforms.length) {
        return [...new Set(state.settings.ownedPlatforms)];
      }
    } catch (_) {}
    return [...FALLBACK_ORDER];
  }

  function hasSource(platform) {
    return platform === 'SWITCH' || !!SQL_FILES[platform];
  }
  ...
  async function fetchSql(platform) {
    ...
    if (!file) return [];
    return parseSql(platform, await fetchText(`${SQL_BASE}${file}`, 12000));
  }
  ...
  async function fetchSwitch() {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      if (!title) continue;
      out.push({key:`stable-SWITCH-${normalize(title)}`, platform:'SWITCH', title, releaseYear:'', genre:'', franchise:'', coverUrl:'', source:'GameTDB'});
    }
  ...
  function progress(done, total, detail, error = false) {
    ...
    el.querySelector('.task-progress-head b').textContent = `${pct}%`;
    el.querySelector('.task-progress-track i').style.width = `${pct}%`;
    el.querySelector('.task-progress-detail').textContent = detail || '';
  }

  function finish(message, error = false) {
    const el = ensureProgress();
    el.classList.toggle('error', error);
    el.querySelector('.task-progress-head span').textContent = error ? 'Каталог не обновлён' : 'Каталог обновлён';
    el.querySelector('.task-progress-head b').textContent = error ? 'Ошибка' : '100%';
    el.querySelector('.task-progress-track i').style.width = '100%';
    el.querySelector('.task-progress-detail').textContent = message;
  ...
  async function refreshStableCatalog() {
    ...
    try {
      ...
      if (!selected.length) {
        finish('Сначала выбери приставки в настройках', true);
        return;
      }
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        progress(i, selected.length, `${name} · ${i + 1} из ${selected.length}`);
        if (!hasSource(platform)) {
          unsupported.push(name);
          progress(i + 1, selected.length, `${name} · источник пока не подключён`);
          continue;
        }
        try {
          ...
          meta[platform] = {updatedAt:new Date().toISOString(), count:mergedPlatform.length, source:platform==='SWITCH'?'GameTDB':'Videogames-Database'};
          successes++;
        } catch (error) {
          console.warn(`Stable catalog refresh ${platform}:`, error);
          failures.push(`${name}: ${error?.message || 'ошибка'}`);
        }
        progress(i + 1, selected.length, `${name} · готово`);
      ...
      const details = [`${successes} из ${selected.length} выбранных платформ обновлено`];
      if (unsupported.length) details.push(`без источника: ${unsupported.join(', ')}`);
      if (failures.length) details.push(`ошибка: ${failures.map(x => x.split(':')[0]).join(', ')}`);
      finish(details.join(' · '));
      try { sessionStorage.setItem('memory-card-catalog-refresh-result', `${successes}/${selected.length}`); } catch (_) {}
  ...
  window.addEventListener('load', () => {
    try {
      ...
      setTimeout(() => {
        ...
        toast.textContent = `Каталог обновлён: ${result} выбранных платформ`;
